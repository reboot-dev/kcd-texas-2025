...
from reboot.aio.memoize import until
...
class AccountServicer(Account.Servicer):
    ...
    async def Interest(
        ...
        await self.state.write(
            f'Increment each iteration #{context.iteration}',
            context,
            increment,
        )

        return Loop(when=timedelta(seconds=random.randint(1, 4)))
...
class BankServicer(Bank.Servicer):
    ...
    async def SignUp(
        ...
        await self.ref().schedule().UpgradeOfferWorkflow(
            context,
    ...
    async def UpgradeOfferWorkflow(
        self,
        context: WorkflowContext,
        request: UpgradeOfferWorkflowRequest,
    ):
        async def large_balance():
            account = Account.ref(request.account_id)
            balance = await account.Balance(context)
            return balance.amount > 1000

        await until('Large balance', context, large_balance)

        # TODO: email the offer!

        print(
            '**************\n'
            f'Account {request.account_id} has sufficient balance for upgrade!'
            '**************\n'
        )

        return UpgradeOfferWorkflowResponse()
...
async def initialize(context: ExternalContext):
    await Bank.idempotently().Create(context, BANK_ID)
...
async def main():
    await Application(
        ...
        # Include `Memoize` servicers.
        reboot.aio.memoize.servicers(),
